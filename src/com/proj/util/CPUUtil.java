...
public class CPUUtil {
	...
	public static boolean hasCompatibleCPU(Context context,String name_Of_SO)
    {

        ElfData elf = readLib(context.getApplicationInfo().dataDir + "/lib/" + name_Of_SO);
        if(elf == null) {
        ...
        if(elf.e_machine == EM_386 && !hasX86) {
        	Log.i(TAG, "x86 build on non-x86 device");
            return false;
        } else if(elf.e_machine == EM_ARM && hasX86) {
        	Log.i(TAG, "ARM build on x86 device");
            return false;
        }

        if(elf.e_machine == EM_MIPS && !hasMips) {
        	Log.i(TAG, "MIPS build on non-MIPS device");
            return false;
        } else if(elf.e_machine == EM_ARM && hasMips) {
        	Log.i(TAG, "ARM build on MIPS device");
            return false;
        }

        if(elf.e_machine == EM_ARM && elf.att_arch.startsWith("v7") && !hasArmV7) {
        	Log.i(TAG, "ARMv7 build on non-ARMv7 device");
            return false;
        }
        if(elf.e_machine == EM_ARM) {
            if(elf.att_arch.startsWith("v6") && !hasArmV6) {
            	Log.i(TAG, "ARMv6 build on non-ARMv6 device");
                return false;
            } else if(elf.att_fpu && !hasFpu) {
            	Log.i(TAG, "FPU-enabled build on non-FPU device");
                return false;
            }
        }
